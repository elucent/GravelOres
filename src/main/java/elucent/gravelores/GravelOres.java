...
public class GravelOres {
	...
	public static void registerAll() {
		...
		blocks.add(coal_gravel_ore = new BlockGravelOre(GRAVEL_ORE, "coal_gravel_ore", "oreCoal", true).setInspiration(Blocks.COAL_ORE)
				.setHarvestProperties("shovel", 0).setHardness(1.6f));
		...
		blocks.add(lapis_gravel_ore = new BlockGravelOre(GRAVEL_ORE, "lapis_gravel_ore", "oreLapis", true).setInspiration(Blocks.LAPIS_ORE)
				.setHarvestProperties("shovel", 1).setHardness(2.0f));
		...
		blocks.add(redstone_gravel_ore = new BlockGravelOre(GRAVEL_ORE, "redstone_gravel_ore", "oreRedstone", true).setInspiration(Blocks.REDSTONE_ORE)
				.setHarvestProperties("shovel", 2).setHardness(2.8f));
		if (!ConfigManager.blacklist.contains("oreRedstone")) {
			addSpawn(redstone_gravel_ore, ConfigManager.weights.get("oreRedstone"));
		}
		blocks.add(diamond_gravel_ore = new BlockGravelOre(GRAVEL_ORE, "diamond_gravel_ore", "oreDiamond", true).setInspiration(Blocks.DIAMOND_ORE)
				.setHarvestProperties("shovel", 2).setHardness(3.0f));
		if (!ConfigManager.blacklist.contains("oreDiamond")) {
			addSpawn(diamond_gravel_ore, ConfigManager.weights.get("oreDiamond"));
		}
		blocks.add(emerald_gravel_ore = new BlockGravelOre(GRAVEL_ORE, "emerald_gravel_ore", "oreEmerald", true).setInspiration(Blocks.EMERALD_ORE)
				.setHarvestProperties("shovel", 2).setHardness(2.8f));
		...
		if (OreDictionary.getOres("oreNickel").size() > 0) {
			blocks.add(nickel_gravel_ore = new BlockGravelOre(GRAVEL_ORE, "nickel_gravel_ore", "oreNickel", true).setHarvestProperties("shovel", 2)
					.setHardness(2.5f));
		...
		if (OreDictionary.getOres("oreSilver").size() > 0) {
			blocks.add(silver_gravel_ore = new BlockGravelOre(GRAVEL_ORE, "silver_gravel_ore", "oreSilver", true).setHarvestProperties("shovel", 2)
					.setHardness(2.6f));
		...
		if (OreDictionary.getOres("oreCopper").size() > 0) {
			blocks.add(copper_gravel_ore = new BlockGravelOre(GRAVEL_ORE, "copper_gravel_ore", "oreCopper", true).setHarvestProperties("shovel", 0)
					.setHardness(1.9f));
					...
					for (int k = 0; k < oreKeys1.length && doContinue; k++) {
						for (int l = 0; l < oreKeys2.length && doContinue; l++) {
							if (oreKeys1[k] == oreKeys2[l]) {
								FurnaceRecipes.instance().addSmeltingRecipeForBlock(blocks.get(i), entry.getValue(),
										FurnaceRecipes.instance().getSmeltingExperience(entry.getValue()));
								doContinue = false;
							}
						}
					}
